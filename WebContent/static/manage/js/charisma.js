...
$(document).ready( function () {
	//菜单的加载
	$.post(_BASE+"/manage/menu.do",function(result){
		var json =  $.parseJSON(result);
		if(json.code==0){
			$("ul.main-menu").append('<li class="nav-header" >功能列表</li>');
			$(json.data).each(function(i,val) { 
				var content = "";
				content += '<li class="accordion">';
				content += '<a href="'+_BASE+val.menuUrl+'"><span>'+val.menuName+'</span></a>';
				if(val.children!=null){
					content += '<ul class="nav nav-pills nav-stacked">';
					$(val.children).each(function(j,child) { 
						content += '	<li><a href="'+_BASE + child.menuUrl+'">'+child.menuName+'</a></li>';
					})
					content += '  </ul>';
				}
				content += '  </li>';
				$("ul.main-menu").append(content);
			});
		}else{
			alert("权限获取失败请重新登录！");
		}
...
function init() {
    ...
    function switchTheme(themeName) {
        if (themeName == 'classic') {
            $('#bs-css').attr('href',_RES_BASE+'bower_components/bootstrap/dist/css/bootstrap.min.css');
        } else {
            $('#bs-css').attr('href',  _RES_BASE+'css/bootstrap-' + themeName + '.min.css');
        }
